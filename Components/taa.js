...
export default class TAA {
  constructor(
    ...
    this.momentMoveRT = DoubleRT(
      positionTexture.image.width,
      positionTexture.image.height,
      THREE.NearestFilter
    );
  ...
  computeMoment(SSRposition) {
    ...
    this.momentBufferMaterial.uniforms.uOldCameraPos.value.set(
      this.lastCameraPos.x,
      this.lastCameraPos.y,
      this.lastCameraPos.z
    );
    this.momentBufferMaterial.uniforms.uLastMomentMove.value =
      this.momentMoveRT.read.texture;
    ...
    for (let i = this.scene.children.length - 1; i >= 0; i--) {
      ...
      if (!this.scene.children[i].oldWorldMatrix)
        this.scene.children[i].oldWorldMatrix =
          this.scene.children[i].matrixWorld.clone();
      ...
      viewModelMatrix.multiplyMatrices(
        this.lastViewMatrixInverse,
        this.scene.children[i].oldWorldMatrix
      );
      this.momentBufferMaterial.uniforms.uOldViewMatrix.value =
        this.lastViewMatrixInverse;
      this.momentBufferMaterial.uniforms.uOldModelViewMatrix.value =
        viewModelMatrix;
      this.momentBufferMaterial.uniforms.uOldModelViewMatrix.needsUpdate = true;
      this.momentBufferMaterial.uniforms.uMeshId.value =
        this.scene.children[i].savedMaterial.meshId;
      ...
      this.momentBufferScene.children[0].material =
        this.momentBufferScene.children[0].savedMaterial;
